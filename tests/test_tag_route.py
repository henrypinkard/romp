...
class _TagRouteHarness(unittest.TestCase):
    ...
    def _post(self, body):
        # km.TOKEN, not os.environ at request time: the kernel binds TOKEN once at import, and a sibling
        # module collected later (test_kernel.py assigns ROMP_SERVE_TOKEN at import) can change what the
        # environment holds by the time a request is sent, so every request here was refused with a 403
        # whenever this module was collected first (2026-09-08; test_color_route.py has the same note).
