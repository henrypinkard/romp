...
def _put_seam_back(prior):
    """The bus reads ROMP_SESSIONS_FILE per CALL and sibling modules bind it once at IMPORT, so every
    class here that binds or pops it puts the PRIOR value back after each test. Popped for good, a
    later module's seam vanished with it: test_postal_read_receipts relayed to a name no listing
    answered for and its five receipt tests failed whenever this module ran ahead (2026-09-08)."""
    if prior is None:
