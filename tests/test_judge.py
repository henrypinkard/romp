...
class Distiller(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # that loads the judge under the name romp_judge shares: a sibling module's failing calls earlier
        # in the same process made the give-up name that model instead of the generic cause (2026-09-08).
        # Each test here starts from a clean latch; tearDown puts the process's own back.
    ...
    def tearDown(self):
        if hasattr(self, "_saved"):
            ...
            jd._PARSE_CACHE.clear(); jd._CHAIN_MEMO.clear()
        if hasattr(self, "_health"):        # DistillSections drives an instance by hand: _setup + tearDown, no setUp
            with jd._health_lock:
                jd._CALL_HEALTH["degraded"].clear()
