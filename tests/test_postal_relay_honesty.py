...
class _RelayBase(unittest.TestCase):
    def setUp(self):
        # The bus reads ROMP_SESSIONS_FILE at CALL time and sibling modules set it once at IMPORT time,
        # so the value popped here (and overwritten by _set_live) is put BACK after each test. Popped for
        # good, every later module's sessions seam vanished with it: test_postal_read_receipts relayed
        # to a name no listing answered for and its five receipt tests failed whenever this module ran
        # ahead of it (2026-09-08).
