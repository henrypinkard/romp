...
class Shell(unittest.TestCase):
    ...
    def test_one_opener_and_every_caller_uses_it(self):
        ...
        _has(self, "window.__rompOpenSettings=function(tab){var f=document.getElementById('f-settings');", js)   # tab (T379): the strip's tab-widgets gear names the Tabs tab
        ...
        _has(self, "if(m.romp==='openSettings')window.__rompOpenSettings(m.tab);", js, "a pane's ask is forwarded, its tab with it (T379)")
        _has(self, "f.contentWindow.postMessage(typeof tab==='string'&&tab?{romp:'openSettings',tab:tab}:{romp:'openSettings'},'*');", js, "the tab rides into the settings iframe; a bare ask stays bare")
