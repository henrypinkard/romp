...
class ExchangeRelaysAreBudgeted(TwoBusExchange):
    ...
    def test_a_413_is_logged_on_every_attempt_where_other_refusals_log_once(self):
        # the dialer's refusal log dedupes by (status, body) so a standing 4xx is one line; a 413 names a
        # request THIS side built too large and re-sends byte for byte, so each attempt is logged
        class NoWait:
            ...
            def wait(self, t=None):
                ...
                if self.n >= self.after:
                    pm.PEERS["srv"]["up"] = False          # the loop sees the peer down and exits

        logged = []
        saved = (pm._peer_http, pm._log, pm._peer_wakes.get("srv"))
        pm._log = lambda msg: logged.append(msg)
        try:
            for code, want in ((413, 3), (400, 1)):
                logged.clear()
                pm.PEERS["srv"] = {"port": 1, "up": True, "trust": "trusted"}
                pm.PEER_STATE.pop("srv", None)
                pm._peer_wakes["srv"] = NoWait(3)

                def refuse(port, payload, token="", code=code):
                    raise urllib.error.HTTPError("http://127.0.0.1:1/peer-exchange", code, "refused", {},
                                                 io.BytesIO(b'{"error": "request body of 1231249 bytes exceeds the 1048576-byte limit"}'))
                pm._peer_http = refuse
                pm._peer_loop("srv")
                lines = [l for l in logged if "exchange refused (HTTP %d)" % code in l]
                self.assertEqual(len(lines), want, (code, logged))
        finally:
            pm._peer_http, pm._log = saved[:2]
            if saved[2] is not None:
                pm._peer_wakes["srv"] = saved[2]
            else:
                pm._peer_wakes.pop("srv", None)
