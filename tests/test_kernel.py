...
class ViewBuilder(unittest.TestCase):
    def setUp(self):
        ...
        # the judge under the name romp_judge: the auto-nudge tests below fail a haiku call apiece through
        # the stubbed CLI, and left in place those failures pushed the model past the give-up cap for the
        # whole run, so test_judge's Distiller give-up then blamed the haiku model instead of naming the
        # generic cause (2026-09-08). Snapshot here, restore in tearDown.
