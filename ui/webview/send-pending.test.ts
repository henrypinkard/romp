...
test("a connection drop marks unconfirmed sends 'not confirmed' — an event, and reversible by a confirmation", () => {
  ...
  assert.match(RENDER, /if \(m\.type === "pipeState"\) \{ if \(!m\.up\) markPendingLost\("connection"\);/,
